...
-- STRATEGY 2: CROSS-SESSION RECENCY (Most recent view_item across sessions)
-- Medium confidence: user viewed item recently, then came back and bought upsell
strategy_2_cross_session AS (
  SELECT
    atc.user_pseudo_id,
    atc.session_id AS atc_session_id,
    atc.upsell_item_id,
...
-- STRATEGY 3: SESSION CO-OCCURRENCE (viewed in same session, before add_to_cart only)
-- Lower confidence: viewed together but not immediately before
strategy_3_co_occurrence AS (
  SELECT
    atc.user_pseudo_id,
    atc.session_id AS atc_session_id,
    atc.upsell_item_id,
...
-- STRATEGY 4: USER-LEVEL FREQUENCY (most viewed item by this user before upsell)
-- Low confidence: based on user's browsing patterns
strategy_4_user_frequency AS (
  SELECT
    user_pseudo_id,
    atc_session_id,
    upsell_item_id,
...
-- STRATEGY 5: GLOBAL FALLBACK (most common parent for this upsell BASE PRODUCT globally)
-- Lowest confidence: statistical fallback when no user-level match found
-- Uses base_product_id for better matching across colors
...
all_matches AS (
  ...
  UNION ALL
  SELECT user_pseudo_id, atc_session_id, upsell_item_id, upsell_base_product_id, add_to_cart_timestamp, parent_item_id, parent_base_product_id, parent_item_name, match_strategy, confidence_score, rn FROM strategy_2_cross_session WHERE rn = 1
  UNION ALL
  SELECT user_pseudo_id, atc_session_id, upsell_item_id, upsell_base_product_id, add_to_cart_timestamp, parent_item_id, parent_base_product_id, parent_item_name, match_strategy, confidence_score, rn FROM strategy_3_co_occurrence WHERE rn = 1
  UNION ALL
  SELECT user_pseudo_id, atc_session_id, upsell_item_id, upsell_base_product_id, add_to_cart_timestamp, parent_item_id, parent_base_product_id, parent_item_name, match_strategy, confidence_score, rn FROM strategy_4_user_frequency WHERE rn = 1
  UNION ALL
  SELECT user_pseudo_id, atc_session_id, upsell_item_id, upsell_base_product_id, add_to_cart_timestamp, parent_item_id, parent_base_product_id, parent_item_name, match_strategy, confidence_score, rn FROM strategy_5_global_fallback
),
