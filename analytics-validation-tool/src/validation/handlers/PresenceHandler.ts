...
export class PresenceHandler implements RuleTypeHandler<PresenceRuleDef> {
    ...
    try {
      ...
      switch (rule.target.type) {
        ...
        case 'event':
          ...
          foundItems = context.getDataLayerEvents(
            rule.target.dataLayerName,
            rule.target.eventName
          );
          matchCount = foundItems.length;
          foundItems.forEach((event: any) => {
            evidence.push({
              type: 'dataLayer',
        ...
        case 'request':
          ...
          matchCount = foundItems.length;
          foundItems.slice(0, 5).forEach((req: any) => {
            evidence.push({
              type: 'request',
        ...
        case 'script':
          ...
          const regex =
            typeof rule.target.urlPattern === 'string'
              ? new RegExp(rule.target.urlPattern, 'i')
              : rule.target.urlPattern;
          foundItems = context.scan.scripts.filter((script) =>
            script.src ? regex.test(script.src) : false
          );
          matchCount = foundItems.length;
          foundItems.forEach((script: any) => {
            evidence.push({
              type: 'script',
        ...
        default:
          throw new Error(`Unknown target type: ${(rule.target as any).type}`);
      }

      // Check count constraints
      const meetsMinCount =
        rule.minCount === undefined || matchCount >= rule.minCount;
      const meetsMaxCount =
        rule.maxCount === undefined || matchCount <= rule.maxCount;
      const meetsExistence =
        rule.shouldExist === true ? matchCount > 0 : matchCount === 0;
