...
exports.createDefaultValidationEngine = createDefaultValidationEngine;
...
class ValidationEngine {
     ...
     */
    async validate(scan, detection, rules) {
        const reportId = (0, uuid_1.v4)();
        const startedAt = Date.now();
        if (this.config.debug) {
            console.log(`[ValidationEngine] Starting validation with ${rules.length} rules`);
        }
        // Build validation context
        const context = (0, ValidationContext_1.buildValidationContext)(scan, detection, this.config.environment);
        ...
        for (const rule of rules) {
            ...
            try {
                ...
                const ruleResults = await this.executeRuleWithTimeout(handler, ruleForHandler, context);
                results.push(...ruleResults);
                if (this.config.debug) {
                    ruleResults.forEach((result) => {
                        console.log(`[ValidationEngine] Rule ${ruleForHandler.id}: ${result.status} - ${result.message}`);
                    });
                }
            }
            catch (error) {
        ...
        if (this.config.debug) {
            console.log(`[ValidationEngine] Validation complete: ${summary.passed} passed, ${summary.failed} failed, ${summary.errors} errors`);
        }
        return {
            id: reportId,
            scanId: scan.id,
     ...
     */
    async executeRuleWithTimeout(handler, rule, context) {
        return Promise.race([
            handler.evaluate(rule, context),
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Rule timeout after ${this.config.ruleTimeout}ms`)), this.config.ruleTimeout)),
        ]);
    }
    /**
 ...
 */
function createDefaultValidationEngine(environment) {
