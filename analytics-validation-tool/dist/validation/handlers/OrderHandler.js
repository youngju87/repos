...
class OrderHandler {
    ...
    async evaluate(rule, context) {
        ...
        try {
            ...
            const meetsTimeConstraint = !rule.maxTimeDifference || timeDifference <= rule.maxTimeDifference;
     ...
     */
    getTimestamp(item, context, evidence) {
        switch (item.type) {
            case 'tag': {
            ...
            case 'script': {
                ...
                const script = context.scan.scripts.find((s) => s.src ? regex.test(s.src) : false);
