...
const DetectorRegistry_1 = require("./DetectorRegistry");
const EvidenceExtractor_1 = require("./EvidenceExtractor");
const timing_1 = require("../core/utils/timing");
const DEFAULT_CONFIG = {
    registry: (0, DetectorRegistry_1.getDefaultRegistry)(),
...
class DetectionEngine {
    ...
    async detect(scan) {
        ...
        const detectionId = (0, types_1.generateId)();
        if (this.config.debug) {
            console.log(`[DetectionEngine] Starting detection for scan: ${scan.id}`);
        }
        // Build evidence context
        timer.mark('evidence-start');
        const evidence = (0, EvidenceExtractor_1.buildEvidenceContext)(scan);
        ...
        for (const detector of detectors) {
            try {
                ...
                const filtered = instances.filter((instance) => instance.confidence >= this.config.minConfidence);
                if (this.config.debug) {
     ...
     */
    async runDetectorWithTimeout(detector, evidence) {
    ...
    mergeInstances(instances) {
        ...
        const allErrors = base.errors ? [...base.errors] : [];
        ...
        const allAdditionalIds = new Set();
        for (const config of configs) {
            if (config.primaryId)
                allPrimaryIds.add(config.primaryId);
        ...
        const mergedConfig = {
            ...
            additionalIds: allPrimaryIds.size > 1
     ...
     */
    calculateCombinedConfidence(evidence) {
 ...
 */
async function detectTags(scan, config) {
