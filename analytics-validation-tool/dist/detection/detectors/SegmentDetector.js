...
exports.SegmentDetector = void 0;
const BaseDetector_1 = require("../BaseDetector");
class SegmentDetector extends BaseDetector_1.BaseDetector {
    id = 'segment';
    name = 'Segment';
    platform = 'segment';
    category = 'cdp';
    ...
    priority = 75;
    // Segment patterns
    SEGMENT_CDN_PATTERN = /cdn\.segment\.(com|io)/i;
    SEGMENT_API_PATTERN = /api\.segment\.(com|io)/i;
    WRITE_KEY_PATTERN = /^[a-zA-Z0-9]{20,}$/;
    ANALYTICS_LOAD_PATTERN = /analytics\.load\s*\(\s*['"]([^'"]+)['"]/;
    ANALYTICS_METHOD_PATTERN = /analytics\.(track|identify|page|group|alias)\s*\(/g;
    /**
    ...
    async detect(context) {
        ...
        const analyticsEvidence = this.detectAnalyticsCalls(context, writeKeys, trackedMethods);
     ...
     */
    detectSegmentScript(context, scriptUrls, writeKeys) {
        const evidence = [];
        for (const url of context.scriptUrls) {
            if (this.SEGMENT_CDN_PATTERN.test(url)) {
                ...
                if (keyMatch && this.WRITE_KEY_PATTERN.test(keyMatch[1])) {
                    ...
                    evidence.push(this.createEvidence('script-url', 'Segment analytics.js with write key', keyMatch[1], BaseDetector_1.CONFIDENCE.HIGH, { scriptUrl: url, writeKey: keyMatch[1] }));
                }
                else {
                    evidence.push(this.createEvidence('script-url', 'Segment CDN script', url, BaseDetector_1.CONFIDENCE.MEDIUM_HIGH, { scriptUrl: url }));
                }
            }
        }
        return evidence;
     ...
     */
    detectAnalyticsCalls(context, writeKeys, trackedMethods) {
        const evidence = [];
        for (const { content, tag } of context.inlineScripts) {
            ...
            if (loadMatch) {
                ...
                if (this.WRITE_KEY_PATTERN.test(key)) {
                    writeKeys.add(key);
                    evidence.push(this.createEvidence('script-content', 'analytics.load call', key, BaseDetector_1.CONFIDENCE.HIGH, { scriptId: tag.id, writeKey: key }));
                }
            }
            ...
            for (const match of methodMatches) {
                ...
                evidence.push(this.createEvidence('script-content', `analytics.${method} call`, method, BaseDetector_1.CONFIDENCE.MEDIUM, { scriptId: tag.id, method }));
            }
            // Check for Segment snippet structure
            if (content.includes('analytics') &&
                content.includes('segment') &&
                content.includes('load')) {
                evidence.push(this.createEvidence('script-content', 'Segment installation snippet', 'segment snippet', BaseDetector_1.CONFIDENCE.MEDIUM, { scriptId: tag.id }));
            }
        }
        return evidence;
    }
     ...
     */
    detectApiCalls(context, endpoints, requestIds) {
        const evidence = [];
        ...
        for (const request of apiRequests) {
            ...
            evidence.push(this.createEvidence('network-endpoint', `Segment ${endpointType} API call`, request.url, BaseDetector_1.CONFIDENCE.HIGH, {
    ...
    detectCookies(context) {
        ...
        if (context.hasCookie('ajs_user_id')) {
            evidence.push(this.createEvidence('cookie', 'ajs_user_id cookie', 'ajs_user_id', BaseDetector_1.CONFIDENCE.MEDIUM, {
                cookieName: 'ajs_user_id',
            }));
        }
        // ajs_anonymous_id cookie
        if (context.hasCookie('ajs_anonymous_id')) {
            evidence.push(this.createEvidence('cookie', 'ajs_anonymous_id cookie', 'ajs_anonymous_id', BaseDetector_1.CONFIDENCE.MEDIUM, { cookieName: 'ajs_anonymous_id' }));
        }
        return evidence;
    }
}
exports.SegmentDetector = SegmentDetector;
