...
exports.getDefaultBrowserManager = getDefaultBrowserManager;
exports.shutdownDefaultBrowserManager = shutdownDefaultBrowserManager;
...
class BrowserManager {
    ...
    async launchBrowser() {
        ...
        try {
            ...
            throw new errors_1.BrowserError(`Failed to launch ${launchConfig.browserType} browser: ${error instanceof Error ? error.message : String(error)}`, { launchConfig });
        }
    ...
    getBrowserType(type) {
        switch (type) {
            ...
            default:
                throw new errors_1.BrowserError(`Unsupported browser type: ${type}`);
        }
    }
    /**
    ...
    async acquireContext(options = {}) {
        if (this.isShuttingDown) {
            throw new errors_1.BrowserError('Browser manager is shutting down');
        }
        if (!this.initialized) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => {
                        ...
                        if (index !== -1) {
                            ...
                            reject(new errors_1.BrowserPoolExhaustedError('Timed out waiting for available browser'));
                        }
                    }, 30000);
                });
            }
     ...
     */
    async createContext(managedBrowser, options) {
        ...
        try {
            ...
            throw new errors_1.BrowserError(`Failed to create browser context: ${error instanceof Error ? error.message : String(error)}`, { browserId: managedBrowser.id, options });
        }
    ...
    async shutdown() {
        ...
        for (const waiter of this.waitQueue) {
            waiter.reject(new errors_1.BrowserError('Browser manager is shutting down'));
        }
 ...
 */
let defaultManager;
function getDefaultBrowserManager() {
    if (!defaultManager) {
        defaultManager = new BrowserManager();
    }
    return defaultManager;
}
async function shutdownDefaultBrowserManager() {
    if (defaultManager) {
