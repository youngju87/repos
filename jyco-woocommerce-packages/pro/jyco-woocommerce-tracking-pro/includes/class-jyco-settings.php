...
class JYCO_Settings {
	...
	public function register_settings() {
		...
		add_settings_section(
			...
			__( 'General Settings', 'jyco-tracking-pro' ),
		...
		add_settings_field(
			'jyco_measurement_id',
			__( 'GA4 Measurement ID', 'jyco-tracking-pro' ),
		...
		// GA4 API Secret (Pro)
		register_setting( 'jyco_tracking_settings', 'jyco_api_secret', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => '',
		) );
		...
		add_settings_field(
			'jyco_api_secret',
			__( 'GA4 API Secret', 'jyco-tracking-pro' ),
		...
		add_settings_field(
			'jyco_debug_mode',
			__( 'Debug Mode', 'jyco-tracking-pro' ),
		...
		add_settings_section(
			'jyco_pro_section',
			__( 'Pro Features', 'jyco-tracking-pro' ),
			array( $this, 'pro_section_callback' ),
			'jyco-tracking-settings'
		);

		// Server-side tracking enabled
		register_setting( 'jyco_tracking_settings', 'jyco_server_side_enabled', array(
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'default'           => true,
		) );
		...
		add_settings_field(
			'jyco_server_side_enabled',
			__( 'Server-Side Tracking', 'jyco-tracking-pro' ),
			array( $this, 'server_side_callback' ),
			'jyco-tracking-settings',
			'jyco_pro_section'
		);

		// Consent mode
		register_setting( 'jyco_tracking_settings', 'jyco_consent_mode', array(
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default'           => 'none',
		) );
		...
		add_settings_field(
			'jyco_consent_mode',
			__( 'Consent Mode', 'jyco-tracking-pro' ),
			array( $this, 'consent_mode_callback' ),
			'jyco-tracking-settings',
			'jyco_pro_section'
		);
	...
	public function debug_mode_callback() {
		...
		<label>
			...
			>
			<?php esc_html_e( 'Enable debug logging in browser console', 'jyco-tracking-pro' ); ?>
		</label>
		<p class="description">
			<?php esc_html_e( 'Shows detailed event data in browser console. Disable in production.', 'jyco-tracking-pro' ); ?>
		</p>
		<?php
	}

	/**
	 * API Secret field (Pro)
	 */
	public function api_secret_callback() {
		$value = get_option( 'jyco_api_secret', '' );
		?>
		<input
			type="password"
			name="jyco_api_secret"
			value="<?php echo esc_attr( $value ); ?>"
			class="regular-text"
			placeholder="Enter API Secret"
		>
		<p class="description">
			<?php esc_html_e( 'Required for server-side tracking. Generate in GA4: Admin → Data Streams → Measurement Protocol API secrets', 'jyco-tracking-pro' ); ?>
		</p>
		<?php
	}
	...
	public function pro_section_callback() {
		echo '<p>' . esc_html__( 'Advanced Pro features for enhanced tracking and compliance.', 'jyco-tracking-pro' ) . '</p>';
	}

	/**
	 * Server-side tracking field
	 */
	public function server_side_callback() {
		$value = get_option( 'jyco_server_side_enabled', true );
		?>
		<label>
			<input
				type="checkbox"
				name="jyco_server_side_enabled"
				value="1"
				<?php checked( $value, true ); ?>
			>
			<?php esc_html_e( 'Enable server-side purchase tracking (backup)', 'jyco-tracking-pro' ); ?>
		</label>
		<p class="description">
			<?php esc_html_e( 'Sends purchase events directly to GA4 via Measurement Protocol as a backup. Requires Measurement ID and API Secret above.', 'jyco-tracking-pro' ); ?>
		</p>
		<?php
	}
	 ...
	 * Consent mode field
	 */
	public function consent_mode_callback() {
		$value = get_option( 'jyco_consent_mode', 'none' );
		$options = array(
			'none'          => __( 'None (No Consent Management)', 'jyco-tracking-pro' ),
			'cookiebot'     => __( 'Cookiebot', 'jyco-tracking-pro' ),
			'complianz'     => __( 'Complianz', 'jyco-tracking-pro' ),
			'cookie_notice' => __( 'Cookie Notice', 'jyco-tracking-pro' ),
			'borlabs'       => __( 'Borlabs Cookie', 'jyco-tracking-pro' ),
			'manual'        => __( 'Manual/Custom Implementation', 'jyco-tracking-pro' ),
		);
		?>
		<select name="jyco_consent_mode" class="regular-text">
		...
		<p class="description">
			<?php esc_html_e( 'Select your consent management platform for Google Consent Mode v2 integration.', 'jyco-tracking-pro' ); ?>
		</p>
		<?php
	}
